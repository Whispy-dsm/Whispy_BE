...
import whispy_server.whispy.domain.notification.application.service.MarkNotificationAsReadService;
...
/**
 * MarkNotificationAsReadService의 단위 테스트 클래스
 * <p>
 * 알림 읽음 처리 서비스를 검증합니다.
 * 소유권 검증 및 알림 읽음 처리 로직을 테스트합니다.
 * </p>
 */
@ExtendWith(MockitoExtension.class)
@DisplayName("MarkNotificationAsReadService 테스트")
class MarkNotificationAsReadServiceTest {
    ...
    @Test
    @DisplayName("본인의 알림을 읽음 상태로 변경할 수 있다")
    void execute_marksOwnNotificationAsRead() {
        // given
        User user = createUser();
        Notification notification = createUnreadNotification(TEST_EMAIL);
        ...
        // then
        verify(saveNotificationPort).save(argThat(savedNotification ->
                savedNotification.id().equals(NOTIFICATION_ID) &&
    ...
    @Test
    @DisplayName("다른 사용자의 알림을 읽음 처리하려 하면 NotificationNotFoundException이 발생한다")
    void execute_throwsException_whenNotOwner() {
        // given
        User user = createUser();
        Notification notification = createUnreadNotification("other@example.com");
    ...
    @Test
    @DisplayName("이미 읽은 알림도 읽음 처리가 가능하다")
    void execute_marksAlreadyReadNotification() {
        // given
        User user = createUser();
        Notification notification = createReadNotification(TEST_EMAIL);
     ...
     */
    private Notification createUnreadNotification(String email) {
    ...
    /**
     * 테스트용 읽은 Notification 객체를 생성합니다.
     *
     * @param email 알림 수신자 이메일
     * @return 생성된 Notification 객체
     */
    private Notification createReadNotification(String email) {
        return new Notification(
                NOTIFICATION_ID,
                email,
                "테스트 제목",
