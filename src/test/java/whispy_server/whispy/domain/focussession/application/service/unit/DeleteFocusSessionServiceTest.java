...
import whispy_server.whispy.domain.focussession.application.service.DeleteFocusSessionService;
...
/**
 * DeleteFocusSessionService의 단위 테스트 클래스
 *
 * 집중 세션 삭제 서비스의 다양한 시나리오를 검증합니다.
 * 세션 삭제 및 소유권 검증 로직을 테스트합니다.
 *
 */
@ExtendWith(MockitoExtension.class)
@DisplayName("DeleteFocusSessionService 테스트")
class DeleteFocusSessionServiceTest {

    @InjectMocks
    private DeleteFocusSessionService deleteFocusSessionService;
    ...
    void whenSessionExists_thenDeletesSuccessfully() {
        ...
        // when
        deleteFocusSessionService.execute(TEST_SESSION_ID);

        // then
        verify(deleteFocusSessionPort).deleteById(TEST_SESSION_ID);
        verify(statisticsCacheVersionManager).bumpUserVersionAfterCommit(TEST_USER_ID, StatisticsCacheDomain.FOCUS);
    }
    ...
    void whenSessionNotExists_thenThrowsException() {
        ...
        // when & then
        assertThatThrownBy(() -> deleteFocusSessionService.execute(TEST_SESSION_ID))
                .isInstanceOf(FocusSessionNotFoundException.class);
        verify(deleteFocusSessionPort, never()).deleteById(TEST_SESSION_ID);
    }
