...
import whispy_server.whispy.domain.user.application.service.ChangePasswordService;
...
/**
 * ChangePasswordService의 단위 테스트 클래스
 * <p>
 * 비밀번호 변경 서비스의 다양한 시나리오를 검증합니다.
 * 이메일 인증 확인, 비밀번호 암호화 및 Redis 상태 삭제를 테스트합니다.
 * </p>
 */
@ExtendWith(MockitoExtension.class)
@DisplayName("ChangePasswordService 테스트")
class ChangePasswordServiceTest {
    ...
    @Test
    @DisplayName("이메일 인증이 완료된 경우 비밀번호를 변경한다")
    void whenEmailVerified_thenChangesPassword() {
        // given
        ChangePasswordRequest request = new ChangePasswordRequest(TEST_EMAIL, NEW_PASSWORD);
    ...
    void whenEmailNotVerified_thenThrowsException() {
        ...
        // when & then
        assertThatThrownBy(() -> changePasswordService.execute(request))
    ...
    @Test
    @DisplayName("비밀번호 변경 시 새 비밀번호를 암호화한다")
    void whenChangingPassword_thenEncodesNewPassword() {
        // given
        ChangePasswordRequest request = new ChangePasswordRequest(TEST_EMAIL, NEW_PASSWORD);
        User user = createUser();
        String statusKey = "email:verification:status:" + TEST_EMAIL;
    ...
    @Test
    @DisplayName("비밀번호 변경 후 Redis 인증 상태를 삭제한다")
    void whenPasswordChanged_thenDeletesVerificationStatus() {
        // given
        ChangePasswordRequest request = new ChangePasswordRequest(TEST_EMAIL, NEW_PASSWORD);
        User user = createUser();
        String statusKey = "email:verification:status:" + TEST_EMAIL;
